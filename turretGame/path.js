...
class PathBetter {
    ...
    setCurrentPoint(){
        this.currentPoint = Interpolator.catmullRom(
    ...
    forward(){
        this.percentBetweenTargets += 0.01;
...
var pathPosBetter= new PathBetter(points);

function move() {
    if (keyBoard.isDown(KeyCode.UP_ARROW)) {
  //      pathPos.forward();
        pathPosBetter.forward();
    }
    if (keyBoard.isDown(KeyCode.DOWN_ARROW)) {
    //    pathPos.backward();
    pathPosBetter.backward();
    }
}
...
function draw() {
    ...
    for(var i = 0; i < pathPosBetter.points.length; i++){
        drawCircle(pathPosBetter.points[i]);
    }
    gc.closePath();
    gc.stroke();
    ...
    gc.beginPath();
    gc.fillStyle = "red";
    drawCircle(pathPosBetter.splineHelperBehind);
    gc.fill();
    gc.closePath();
    // draw target behind
    gc.beginPath();
    gc.fillStyle = "green";
    drawCircle(pathPosBetter.targetBehind);
    gc.fill();
    gc.closePath();
    // draw current position
    gc.beginPath();
    gc.fillStyle = "blue";
    drawCircle(pathPosBetter.currentPoint);
    gc.fill();
    gc.closePath();
    // draw target forward
    gc.beginPath();
    gc.fillStyle = "green";
    drawCircle(pathPosBetter.targetForward);
    gc.fill();
    gc.closePath();
    // draw spline helper forward
    gc.beginPath();
    gc.fillStyle = "red";
    drawCircle(pathPosBetter.splineHelperForward);
    gc.fill();
    gc.closePath();
}
